...
var SESSION_KEY = "miyaid";
var TARGET_SERVER_URL = "http://uploads.miyabaobei.com/upload_web_koubei.php";
...
function modify_file(num)
{
    ...
    // url = 'http://img.miyabaobei.com/'+url;
    url = 'http://img.miyabaobei.com/'+url;
    $("#mtBlock").show();
...
function uploadSuccess(file, serverData) {
    if (serverData.indexOf("FILEID:") >= 0) {
        ...
        if (this.callback_success) {
            ...
            // var imgs_path = 'http://img.miyabaobei.com/'+ss_con[1];    // for test
            var imgs_path = 'http://img.miyabaobei.com/'+ss_con[1];
...
UploadApp = {
    init:function(container_id,value_container_id, max_file, privacy, file_type, file_type_description, img_type, resource_id, callback_success, callback_delete, show_queue){
    	...
    	var settings = {
    	    ...
    	    // Flash Settings
    	    flash_url : "http://uploads.miyabaobei.com/flash/swfupload.swf",	   
...
//    	return "m32b3a34a59m5fos4p0lip48s5";
	...
	if (!sessionid)
	    sessionid = jQuery.get_cookie('miyaid');
	return sessionid;
    }
};
