...
@section('content')
    ...
    <div class="photo l">
                <img src="{{url(config('app.userUrl').$res -> header)}}" alt="默认头像" width="120">
                <a href="{{url('/user/prefile/photo')}}">修改头像</a></div>
    <div class="intro l">
        <div class="tit01">{{ session('user.username') }}, 欢迎您! </div>
        <p>会员等级：{{ $grade }}  <a href="{{ url('/help/4') }}" target="_blank" class="gray">会员级别介绍</a><br>
        当前积分：<span class="pink">{{ $score }}</span>  <a href="{{ url('/help/5') }}" target="_blank" class="gray">积分获取规则</a> </p>
        <div class="iline"></div>
        <p> 累计消费金额：
            <span class="pink">¥ {{ $sum }}</span><br>
            </p><div class="iline"></div>
            <span id="accountStatus">
                <!--<span class="verify">
                                        手机已验证
                                    </span>
                <span class="verify">
                                        邮箱未验证 <a href="http://local.naiya.cc/my_emailverify" class="gray">去验证&gt;&gt;</a>&nbsp;&nbsp;[送50奶豆]
                                    </span>-->
                <!--<span class="verify">
					未实名认证 <a href="http://local.naiya.cc/my_identification.html" class="gray">去验证&gt;&gt;</a>
                </span>-->                
            </span>
        <p></p>
    </div>
...
<div class="splist p20" id="orderList">
            ...
            <table class="tab04" width="100%" cellspacing="0" cellpadding="0" border="0">
             ...
             <div class="corder_childtd clearfix">
                ...
                <img src="{{ url(config('app.goodsUrl').$key -> images) }}" alt="{{ $key -> goodsname }}" title="{{ $key -> goodsname }}" width="50" height="50">
                                                        </a>
                 <a href="{{ url('/goods/') }}/{{ $key -> goodsid }}" target="_blank">{{ $key -> goodsname }}</a>
                 <p class="orderin">
