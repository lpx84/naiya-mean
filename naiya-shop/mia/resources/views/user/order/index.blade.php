	...
	<div class="right l">
                <div class="title yahei f20 pink">
                    <ul class="tab-tit clearfix">
                        <li class="cur dhref" href="/order_lists"><a href="{{ url('/user/order') }} " style="color: #fff;">全部订单</li>
                        <li class=" dhref" dhref="/order_lists?s=nopay"><a href="{{ url('/user/order/payment') }} ">待付款<em>（{{ $paymentNum -> count }}）</em></a></li>
                        <li class=" dhref" dhref="/order_lists?s=nosend"><a href="{{ url('/user/order/shipments') }} ">待发货<em>（{{ $shipmentsNum -> count }}）</em></a></li>
                        <li class=" dhref" dhref="/order_lists?s=hassend"><a href="{{ url('/user/order/doshipments') }} ">已发货<em>（{{ $doshipmentsNum -> count }}）</em></a></li>
                    </ul>
                </div>
                ...
                <table class="msctab outer" width="100%" cellspacing="0" cellpadding="0" border="0">
				   ...
				   <tr class="orderListHead">
					<th colspan="4" class="ddtit" align="left">
						<span>订单号：<a href="">{{ $extKey['order_no'] }}</a></span>
					<span>下单时间：{{ date('Y-m-d H:i:s',$extKey['create_at']) }}</span>
					@if($extKey['status'] == 1)
					<span>
						<a href="{{ url('/user/details') }}/{{ $extKey['id'] }}" target="_blank" class="changeAddressBtn r" style="background:#f450a2; color:#fff; width:50px;">订单详情</a>
					</span>
					<span>
						<a href="{{ url('/user/cartOrder/oid') }}/{{ $extKey['id']}}" data-oc="20160620591306792" class="changeAddressBtn r" style="background:#f450a2; color:#fff; width:50px;"><b>去支付</b></a>
					</span>
                    <span>
						
						<a href="{{ url('/user/update') }}/{{ $extKey['id'] }}" data-oc="20160620591306792" data-c="0" class="cancelOrder pink r">取消订单</a>
					</span>
					@endif
                        </th>
                    </tr>
					@if($extKey['sub'])
            @foreach($extKey['sub'] as $y => $key) 
             ...
             <div class="corder_childtd clearfix">
                ...
                <span class="gray listNum">数量：{{ $key -> num }}</span>
                                    ...
                                    <td class="f14 pink" width="15%" align="center">¥ {{ $key -> stdprice * $key -> num }}</td>
									@if($y == 0)
                                    <td colspan="2" rowspan="{{count($extKey['sub'])}}" width="15%" align="center" style="color: #666;">
									{{ $arr[$key -> status] }}
                                    </td>
									@endif
                    </tr>
